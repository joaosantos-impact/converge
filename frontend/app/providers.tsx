...
function getCachedRates(): { EUR: number; BTC: number } {
  if (typeof window === 'undefined') return DEFAULT_RATES;
  try {
    const cached = localStorage.getItem(RATES_CACHE_KEY);
    if (cached) {
      const { rates: cachedRates, timestamp } = JSON.parse(cached);
      if (Date.now() - timestamp < RATES_REFRESH_MS * 6) return cachedRates;
    }
  } catch { /* ignore */ }
  return DEFAULT_RATES;
}

async function fetchExchangeRates(): Promise<{ EUR: number; BTC: number }> {
...
function CurrencyProvider({ children }: { children: React.ReactNode }) {
  ...
  const { data: rates = DEFAULT_RATES } = useQuery({
    ...
    initialData: getCachedRates,
    staleTime: RATES_REFRESH_MS,
